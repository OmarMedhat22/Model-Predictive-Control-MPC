...
for (int t = 1; t < N; t++) {
  ...
  fg[1 + psi_start + t] = psi1 - (psi0 - v0 * delta0*latency/ Lf * dt)  ; //////////////////////////////////////
  fg[1 + v_start + t] = v1 - (v0 + a0 * dt*latency);/////////////////////////////////////////////////////////////
  fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt)*latency) ;
  fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 * delta0*latency / Lf * dt);
}
...
double epsi = state[5];
  ...
  for (int i = a_start; i < n_vars; i++) {
   vars_lowerbound[i] = -0.436332*Lf; //-1.6;
    vars_upperbound[i] = 0.436332*Lf; //1.6;
    cout << "4";
  }
